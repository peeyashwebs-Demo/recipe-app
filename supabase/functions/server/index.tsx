...
import { seedCooks, seedRecipes, seedReviews, categories } from "./seed_data.tsx";

// Generates a random, non-guessable password (used only for seeded demo
// accounts — real users always choose their own password at signup).
...
// The ONE account allowed to approve/reject recipes, feature them, or
// moderate reviews. This is checked against the authenticated user's real
// email on every request — not a role that can be granted or stored client
// side — so it can't be widened by anything short of editing this constant.
const ADMIN_EMAIL = "peeyeshweb@gmail.com";
...
async function ensureSeeded() {
  ...
  for (const cook of seedCooks) {
    ...
    if (data?.user) {
      id = data.user.id;
      generatedCredentials.push({ email: cook.email, password, role: cook.role });
    } else {
  ...
  if (generatedCredentials.length) {
    // Printed once, only to this function's private Supabase logs — never
    // returned to the client or stored anywhere the frontend can read.
    console.log("Seed: generated credentials for new seeded accounts (visible only in these function logs):");
    for (const cred of generatedCredentials) {
      console.log(`  ${cred.role.padEnd(7)} ${cred.email}  password: ${cred.password}`);
    }
    console.log("Seed: save these now — they are not logged again after this run.");
  }
...
// Sign up (creates auth user + profile). Always creates a regular "user" —
// admin is a trusted role and is never granted through public signup.
app.post(`${P}/signup`, async (c) => {
  try {
    ...
    await kv.set(`profile:${data.user.id}`, {
      ...
      avatar: `https://i.pravatar.cc/160?u=${encodeURIComponent(email)}`, role: isAdminEmail(email) ? "admin" : "user",
    });
...
app.get(`${P}/me`, async (c) => {
  ...
  // The admin role is derived from ADMIN_EMAIL on every load — this
  // self-corrects stale data (e.g. if it was ever set some other way) so
  // there is never more than one admin account, and it's always this one.
  const correctRole = isAdminEmail(user.email) ? "admin" : profile.role === "admin" ? "user" : profile.role;
...
// Update recipe status (admin only — the creator, even the recipe's own
// owner, cannot approve/reject their own submission).
app.patch(`${P}/recipes/:id/status`, async (c) => {
  const user = await getUser(c);
  if (!user) return c.json({ error: "Unauthorized" }, 401);
  if (!isAdminEmail(user.email)) return c.json({ error: "Forbidden" }, 403);
...
app.delete(`${P}/recipes/:id`, async (c) => {
  ...
  const recipe = await kv.get(`recipe:${c.req.param("id")}`);
  if (!recipe) return c.json({ error: "Not found" }, 404);
  if (recipe.creatorId !== user.id && !isAdminEmail(user.email)) return c.json({ error: "Forbidden" }, 403);
